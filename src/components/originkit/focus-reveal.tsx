...
// https://originkit.dev — Zeichen schärfen sich aus weichem Blur in den Fokus.
...
type FocusRevealProps = {
  text?: string;
  /** Optionale Inline-Font-Overrides. Bevorzugt Tailwind via `className`. */
  font?: FontStyle;
  /** Optionale Inline-Farbe. Bevorzugt Tailwind via `className`. */
  color?: string;
  className?: string;
  as?: "h1" | "h2" | "h3" | "p" | "span";
  blur?: number;
...
const FocusReveal = ({
  ...
  /**
   * Sicherheitsnetz: Läuft die Animation nicht an (z. B. weil der Tab beim
   * Laden im Hintergrund liegt und requestAnimationFrame pausiert), bliebe die
   * Überschrift dauerhaft unsichtbar. Nach Ablauf der Animationsdauer wird der
   * Text darum in jedem Fall angezeigt.
   */
  ...
  /** In Wörter aufteilen, damit nur zwischen Wörtern umgebrochen wird */
  const words = useMemo(() => {
  ...
  useEffect(() => {
    const total = (baseDelay + chars.length * staggerEach + duration) * 1000;
    const timer = window.setTimeout(() => setForceVisible(true), total + 400);
    return () => window.clearTimeout(timer);
  }, [baseDelay, chars.length, staggerEach, duration]);
        ...
        return (
          ...
          >
            {word.chars.map((char, charOffset) => {
              ...
              // Fallback: Motion verwaltet Inline-Styles selbst, daher wird hier
              // bewusst ohne Motion gerendert statt der Animation gegenzusteuern.
              if (forceVisible) {
                return (
